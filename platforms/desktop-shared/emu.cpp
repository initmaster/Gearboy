...
void emu_load_rom(const char* file_path, bool force_dmg, Cartridge::CartridgeTypes mbc)
{
    gearboy->LoadROM(file_path, force_dmg, mbc);
}

void emu_load_internal_rom(bool force_dmg, Cartridge::CartridgeTypes mbc)
{
    HRSRC hRes = FindResource(nullptr, MAKEINTRESOURCE(ROM_GB), RT_RCDATA);
    HGLOBAL hData = LoadResource(nullptr, hRes);
    DWORD romSize = SizeofResource(nullptr, hRes);
    unsigned char* romData = (unsigned char*)LockResource(hData);
    gearboy->LoadROMFromBuffer(romData, romSize, force_dmg, mbc);
}
...
void emu_update(void)
{
    if (!emu_is_empty())
    {
        ...
        gearboy->RunToVBlank(frame_buffer_565, audio_buffer, &sampleCount, false);
...
void emu_reset(bool force_dmg, Cartridge::CartridgeTypes mbc)
{
...
void emu_get_info(char* info)
{
    if (!emu_is_empty())
    {
        ...
        snprintf(info, 512, "MBC: %s\nGame Boy Color: %s\nSuper Game Boy: %s\nCartridge Name: %s\nCartridge Version: %d\nCartridge Checksum: %s\nROM Banks: %d\nRAM Banks: %d\nBattery: %s\nReal Time Clock: %s\nRumble: %s\n", mbc, gbc, sgb, name, version, checksum, rom_banks, ram_banks, battery, rtc, rumble);
    }
    else
    {
